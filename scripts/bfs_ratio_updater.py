...
search_list = ['bfsdc','bfsdcu','astar','greedy']
...
def create_unique_index(collection):
    collection.create_index(
        [("domain_name", 1), ("problem_name", 1), ("init_name", 1), ("search",1)],
        unique=True
    )
...
if __name__ == '__main__':
    ...
    ratio_collection = my_db['bfs_ratio']
    ...
    # }
    create_unique_index(ratio_collection)
    
    bfs_query = {'search':'bfs'}    
    for item in my_collection.find(bfs_query):
        domain_name = item['domain_name']
        problem_name = item['problem_name']
        init_name = item['init_name']
        bfs_search_time = item['search_time']
        all_search_exist = True
        updating_list = []
        for search_name in search_list:
            query = {'domain_name':domain_name,'problem_name':problem_name,'init_name':init_name,'search':search_name}
            found = my_collection.find_one(query)
            if found == None:
                all_search_exist = False
                break
            else:
                updating_list.append(found)
        if all_search_exist:
            for record in updating_list:
                record['bfs_ratio_search_time'] = record['search_time']/bfs_search_time
                try:
                    ratio_collection.insert_one(record)
                    # print("Document inserted successfully")
                except pymongo.errors.DuplicateKeyError:
                    print("Duplicate document found. Insertion skipped.")
                    print(record)
    delete_unique_index(ratio_collection)
                    
    
            # ratio_item = ratio_collection.find_one(query)
            # if ratio_item == None:
            #     all_search_exist = False
            #     break
        # search_name = item['search']
        # query = {'domain_name':domain_name,'problem_name':problem_name,'init_name':init_name,'search':search_name}
        # ratio_item = ratio_collection.find_one(query)
        # if ratio_item == None:
        #     print("Not found, adding: ",problem_name)
        #     ratio_collection.insert_one(item)
        # else:
        #     print("Found, skipping: ",problem_name)
