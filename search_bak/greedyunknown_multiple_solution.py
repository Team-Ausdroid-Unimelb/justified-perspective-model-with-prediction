...
class Search:
    ...
    def searching(self,problem, filterActionNames = None):
        ...
        while not open_list.isEmpty():
            ...
            if is_goal and temp_solution_length == 0:
                
                temp_solution_length = len(path) -1
                # print(f"temp goal found with len: {len(path) -1}")
                actions = [ a  for s,a in path]
                actions = actions[1:]
                self.logger.info(f'temp plan is: {actions}')
                self.logger.info(f'temp goal found with len: {len(path) -1}')
                self.result.update({'solvable': True})
                self.result.update({'plan':actions})
                continue
            elif is_goal and temp_solution_length <= len(path) -1:
                self.logger.info(f'temp goal found with len: {len(path) -1}')
                # print(f"temp goal 2 found with len: {len(path) -1}")
                continue
