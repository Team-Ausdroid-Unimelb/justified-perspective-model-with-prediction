        ...
        while len(queue):
            ...
            if 1:
                ...
                for action in filtered_action_names:
                    pre_flag,temp_p_dict_pre = problem.checkPreconditionsP(state,actions[action],path)
                    if pre_flag: 
                        ...
                        _, temp_p_dict_goal = problem.isGoalP(succ_state,path + [(succ_state,action)])
                        temp_p_dict_pre.update(temp_p_dict_goal)
                        if not temp_p_dict_pre in self.visited:
                            succ_node = self.SearchNode(succ_state,temp_p_dict,path + [(succ_state,action)])
                            self.generated += 1
                            queue.append(succ_node)
                            self.visited.append(temp_p_dict_pre)
                        else:
                            self.pruned += 1
            else:
                self.pruned += 1
